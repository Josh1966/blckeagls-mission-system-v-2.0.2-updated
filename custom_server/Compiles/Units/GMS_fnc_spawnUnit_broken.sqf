...
private ["_pos","_i","_weap","_ammo","_other","_skin","_aiGroup","_ai1","_magazines","_players","_owner","_ownerOnline","_nearEntities","_skillLevel","_aiSkills",
		"_Launcher","_launcherRound","_vest","_index","_weaponList","_ammoChoices","_attachment","_attachments",
		"_headGear","_uniforms"];

//_shemag = ["H_Shemag_khk","H_Shemag_olive","H_Shemag_tan","H_ShemagOpen_khk"];
params["_pos","_weaponList","_aiGroup",["_skillLevel","red"],["_Launcher","none"],["_uniforms",blck_SkinList],["_headGear",blck_BanditHeadgear]];
/*
_pos = param[0]; //_this select 0;  // Position at which to spawn AI
_weaponList = param[1]; //_this select 1;
_aiGroup = param[2]; //_this select 2;  // Group to which AI belongs
_skillLevel = param[3,"red"]; //[_this,3,"red"] call BIS_fnc_param;   // Assign a skill level in case one was not passed."blue", "red", "green", "orange"
_Launcher = param[4,"none"];// [_this, 4, "none"] call BIS_fnc_param; // Set launchers to "none" if no setting was passed.
_uniforms = param[5,blck_SkinList]; //[_this, 5, blck_SkinList] call BIS_fnc_param;
_headGear =  param[6,blck_BanditHeadgear]; //[_this, 6, blck_BanditHeadgear]  call BIS_fnc_param;//_headGear
*/
_ai1 = ObjNull;
...
while {_allowed == -1} do
{
	...
	_allowed = if (_ai1 isUniformAllowed _skin) then {1} else {-1};
};
...
_ai1 setVariable["SecondaryWeap",_weap];
