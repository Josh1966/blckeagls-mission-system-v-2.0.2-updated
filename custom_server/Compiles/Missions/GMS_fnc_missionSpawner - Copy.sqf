...
_markerClass = _this select 1; // Currently Supported Marker types are: "Blue","Red","Green","Orange","ScoutsMarker","HunterMarker","HCMarker","RogueMarker"
_aiDifficultyLevel = _this select 2; // "blue","red","green" and "orange"
_missionEnabled = 1 to enable, -1 to disable; added to provide a simple way to enable or disable specific mission classes through blck_custom_config.sqf 
*/
...
diag_log format["[blckeagls] missionSpawner:: Initializing mission: _cords %1 : _markerClass %2 :  _aiDifficultyLevel %3 _markerMissionName %4",_coords,_markerClass,_aiDifficultyLevel,_markerMissionName];
...
_blck_AllMissionAI = [];
_AI_Vehicles = [];
_blck_localMissionMarker = [_markerClass,_coords,"",""];
_delayTime = 1;
...
if (!_playerInRange && _missionTimedOut) exitWith
{
	...
	if (blck_debugON) then
	{
		diag_log format["[blckeagls] missionSpawner:: Mission Timed Out: _cords %1 : _markerClass %2 :  _aiDifficultyLevel %3 _markerMissionName %4",_coords,_markerClass,_aiDifficultyLevel,_markerMissionName];
	};
};
...
{
	if (blck_debugON) then
	{		diag_log format["[blckeagls] missionSpawner:: --  >>  Mission tripped by nearby player: _cords %1 : _markerClass %2 :  _aiDifficultyLevel %3 _markerMissionName %4",_coords,_markerClass,_aiDifficultyLevel,_markerMissionName];
	};
	...
	{
		_crates = [_coords,[["Box_NATO_Wps_F",[0,0,0],_crateLoot,_lootCounts]]] call blck_fnc_spawnMissionCrates;
	...
	if (blck_debugON) then
	{
		diag_log format["[blckeagls] missionSpawner:: Crates Spawned: _cords %1 : _markerClass %2 :  _aiDifficultyLevel %3 _markerMissionName %4",_coords,_markerClass,_aiDifficultyLevel,_markerMissionName];
	};
	...
	if (blck_debugON) then
	{
		diag_log format["[blckeagls] missionSpawner:: Landscape spawned: _cords %1 : _markerClass %2 :  _aiDifficultyLevel %3 _markerMissionName %4",_coords,_markerClass,_aiDifficultyLevel,_markerMissionName];
	};
	...
	if (blck_useStatic && (_noEmplacedWeapons > 0)) then
	{
		...
		if (blck_debugON) then
		{
			diag_log format["[blckeagls] missionSpawner:: Static Weapons Spawned: _cords %1 : _markerClass %2 :  _aiDifficultyLevel %3 _markerMissionName %4",_coords,_markerClass,_aiDifficultyLevel,_markerMissionName];
		};
	};
	...
	{
		private["_vehiclePatrolGroup","_patrolVehicle","_vehiclePatrolSpawns"];
		...
		{
			_vehiclePatrolGroup = [_x,3,3,_aiDifficultyLevel,_coords,1,2,_uniforms,_headGear] call blck_fnc_spawnGroup;
			//diag_log format["missionSpawner:: group for AI Patrol vehicle spawn: group is %1 with units of %2",_vehiclePatrolGroup, units _vehiclePatrolGroup];
			_blck_AllMissionAI = _blck_AllMissionAI + (units _vehiclePatrolGroup);
			...
			_patrolVehicle = [_x,_randomVehicle,6,25,_vehiclePatrolGroup] call blck_fnc_spawnVehiclePatrol;
			//diag_log format["missionSpawner:: patrol vehicle spawned was %1",_patrolVehicle];
			_vehiclePatrolGroup setVariable["groupVehicle",_patrolVehicle,true];
			uiSleep _delayTime;
	...
	if ((random(1) < _chanceReinforcements)) then
	{
		...
		[] spawn {
			_grpReinforcements = [_coords,_noPara,_aiDifficultyLevel,_chanceLoot,_reinforcementLootCounts,_weaponList,_uniforms,_headgear,_chanceHeliPatrol] spawn blck_fnc_Reinforcements;
	...
	*/
	...
	if (blck_debugON) then
	{
		diag_log format["[blckeagls] missionSpawner:: Mission completion criteria fulfilled: _cords %1 : _markerClass %2 :  _aiDifficultyLevel %3 _markerMissionName %4",_coords,_markerClass,_aiDifficultyLevel,_markerMissionName];
	};
	...
	if (blck_useSignalEnd) then
	{
		...
		if (blck_debugON) then
		{
			diag_log format["[blckeagls] missionSpawner:: SignalEnd called: _cords %1 : _markerClass %2 :  _aiDifficultyLevel %3 _markerMissionName %4",_coords,_markerClass,_aiDifficultyLevel,_markerMissionName];
		};
	};
	...
	diag_log format["[blckeagls] missionSpawner:: end of mission: _cords %1 : _markerClass %2 :  _aiDifficultyLevel %3 _markerMissionName %4",_coords,_markerClass,_aiDifficultyLevel,_markerMissionName];
};
